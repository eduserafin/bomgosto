...
<html>
    ...
    <table width="100%" class="table table-bordered table-striped modern-table">
        <tr>
            ...
            <th class="bg-info"><strong>PERFIL</strong></th>
            <th class="bg-info"><strong>STATUS</strong></th>
            <th class="bg-info"><strong>AÇÕES</strong></th>
        </tr>
        <?php
            $SQL = "SELECT u.nr_sequencial, c.ds_colaborador, u.ds_login, u.ds_email,
                    CASE 
                        WHEN u.st_status = 'A' THEN 'ATIVO'
                        WHEN u.st_status = 'I' THEN 'INATIVO'
                        ELSE 'INDEFINIDO'
                    END AS ds_status,
                    CASE
                        WHEN u.st_admin = 'A' THEN 'ADMINISTRADOR'
                        WHEN u.st_admin = 'G' THEN 'GERENTE'
                        WHEN u.st_admin = 'C' THEN 'COLABORADOR'
                        ELSE 'INDEFINIDO'
                    END AS ds_admin
                    FROM usuarios u
