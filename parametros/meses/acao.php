...
<?php
  ...
  if ($Tipo == "D") {
      ...
      if ($RS["nr_sequencial"] == $Codigo) {
        ...
        echo "<script language='javascript'>window.parent.document.getElementById('cd_meses').value='" . $RS["nr_sequencial"] . "';</script>";
  ...
  if ($Tipo == "I") {
      ...
      if ($RS["nr_sequencial"] >0) {
        ...
        $SQL1 = "SELECT nr_sequencial
                    FROM consorcio_quantidade_mes
                  WHERE nr_sequencial = (SELECT max(nr_sequencial) FROM consorcio_quantidade_mes)
  ...
  if ($Tipo == "E") {
    ...
    if ($v_existe > 0) {
      ...
      $update = "DELETE FROM consorcio_quantidade_mes WHERE nr_sequencial=" . $codigo;
      mysqli_query($conexao, $update);
